...
class IPThrottleMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        if ip in IP_THROTTLER:
            ...
            requests_in_last_minute = len([date for date in IP_THROTTLER[ip] if (now - date) < datetime(minute=1)])
            requests_in_last_day = len([date for date in IP_THROTTLER[ip] if (now - date) < datetime(day=1)])
